...
function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {modal?.type === "userFiatProof" && (
        <EvidenceModal
          title="Submit fiat payment proof"
          label="Bank transfer / fiat receipt hash"
          onClose={() => setModal(null)}
          onSubmit={(hash) => {
            patchPayment(
              selected.id,
              (p) => ({
                ...p,
                userFiatProofHash: hash || nowHash(),
                audit: [...p.audit, "User submitted fiat payment proof"],
              }),
              "User submitted fiat payment proof"
            );
            setModal(null);
          }}
        />
      )}
      {modal?.type === "userCryptoDeposit" && (
        <EvidenceModal
          title="Confirm crypto deposit"
          label="Crypto deposit transaction hash"
          onClose={() => setModal(null)}
          onSubmit={(hash) => {
            patchPayment(
              selected.id,
              (p) => ({
                ...
                fundingStatus: FUNDING_STATUS.READY,
                status: PAYMENT_STATUS.CREATED,
                userCryptoDepositHash: hash || nowHash(),
                audit: [...p.audit, "User confirmed crypto deposit"],
              }),
              "User confirmed crypto deposit"
            );
            setModal(null);
          }}
        />
...
function PaymentList({
  ...
  const filtered = payments
    ...
    .sort((a, b) => {
      if (view === "user") {
        const aMine = a.creator.id === currentUser.id ? 1 : 0;
        const bMine = b.creator.id === currentUser.id ? 1 : 0;
        return bMine - aMine || b.id - a.id;
      }
      return b.id - a.id;
    });
  return (
...
function UserPaymentView({ payment, currentUser, patchPayment, setModal }) {
  const isCreator = payment.creator.id === currentUser.id;

  const userFiatProofNeeded =
    isCreator &&
    [PAYMENT_KIND.FIAT_TO_CRYPTO, PAYMENT_KIND.FIAT_TO_FIAT].includes(
      payment.paymentKind
    ) &&
    payment.fundingStatus === FUNDING_STATUS.QUOTE_SET &&
    !payment.userFiatProofHash;

  const canCreatorDepositCrypto =
    isCreator &&
  ...
  return (
    <div className="space-y-6">
      <UserHeader payment={payment} currentUser={currentUser} />

      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Panel
          title="Payment progress"
          subtitle="Track the payment from creation to claim or settlement."
        >
        ...
        <Panel
          ...
          subtitle="Actions appear only when this payment is waiting for you."
        >
          <div className="space-y-3">
            {userFiatProofNeeded ? (
              <PrimaryAction
                icon={ReceiptText}
                title="Submit fiat payment proof"
                desc={`Upload or paste proof after paying ${money(
                  payment.sourceAmount,
                  payment.route.origin
                )} off-chain.`}
                onClick={() => setModal({ type: "userFiatProof" })}
              />
            ) : null}

            {canCreatorDepositCrypto ? (
              <PrimaryAction
                icon={Wallet}
                title="Confirm crypto deposit"
                desc={`Confirm your ${money(
                  payment.escrowAmount
                )} USDC escrow deposit transaction.`}
                onClick={() => setModal({ type: "userCryptoDeposit" })}
              />
            ) : null}
            ...
            {!isCreator ? (
              <EmptyMessage
                ...
                text="Select your own payment from the list, or switch user from the top bar."
              />
            ) : null}
            ...
            !canCreatorClaim ? (
              <EmptyMessage
                ...
                text="The broker or assigned settlers are processing the next step."
              />
            ) : null}
          </div>
        </Panel>
...
function UserHeader({ payment, currentUser }) {
  return (
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">
            {payment.creator.name} · {shortKind(payment.paymentKind)}
          </p>
        </div>
        <div className="grid gap-3 sm:grid-cols-3">
          <MiniTile
            label="Route"
            value={`${currency(payment.route.origin)} → ${currency(
          ...
          />
          <MiniTile label="Status" value={statusLabel(payment.status)} />
        </div>
      </div>
    </div>
  );
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <CommandButton
              ...
              text={
                payment.userFiatProofHash
                  ? "Submit fiat receipt hash after verifying user proof."
                  : "Waiting for user fiat payment proof."
              }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-2">
      ...
      >
        ...
        />
        <InfoRow
          label="User fiat proof"
          value={payment.userFiatProofHash || "Not submitted"}
          copy={!!payment.userFiatProofHash}
        />
        <InfoRow
          label="User crypto deposit"
          value={payment.userCryptoDepositHash || "Not submitted"}
          copy={!!payment.userCryptoDepositHash}
        />
        <InfoRow
          label="Fiat evidence"
